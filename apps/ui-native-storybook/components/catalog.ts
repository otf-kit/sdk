...
export const CATALOG: Category[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      { slug: 'multistep', title: 'MultiStep', description: 'Declarative wizard — bar/dots/segments progress, slide+fade transitions, per-instance back handler.', status: 'ready' },
      { slug: 'selectable', title: 'Selectable', description: 'Single + multi option cards (card / row / tile variants) with discriminated-union API.', status: 'ready' },
      { slug: 'animated-view', title: 'AnimatedView', description: 'Declarative entrance — 10 presets, stagger, viewport trigger, reduced-motion aware.', status: 'ready' },
      { slug: 'expandable', title: 'Expandable', description: 'Single-row accordion — controlled / uncontrolled, plain + card variants.', status: 'ready' },
      { slug: 'app-header', title: 'App Header', description: 'Top bar — 4 layout variants × 3 surface variants (solid/transparent/blurred) + collapsible.', status: 'ready' },
      { slug: 'card-scroller', title: 'CardScroller', description: 'Snap-to-card horizontal scroller with auto-center, fade-off interpolation, generic data.', status: 'ready' },
      ...
      { slug: 'fab', title: 'Floating Action Button', description: 'Anchored CTA — legacy circle/pill modes + expanding actions menu with backdrop, hardware-back, and per-action accent.', status: 'ready' },
